...
Скрипт для построения ледовых весов для каждого ребра каждого ЛК
...
def parse_velocity(velocity: int):
    ...
    #Входные данные задачи: потери скорости для каждого класса судна в каждом классе льда
    LIMITS = {
        'Arc7': {
            'provided': {
                'medium': 1,
...
def get_total_line_time_coefficient(a, b, line_length):
    ...
    for i in range(amount_of_pieces):
        ...
        intVel =IntegrValues[( lattitude, longitude)]
        tmp = parse_velocity(intVel)
        ...
        if intVel >= 19.5:
            lines_icebreakers['Arc91']['clear']+=piece_length
            lines_icebreakers['Arc92']['clear']+=piece_length
        elif intVel >= 14.5:
            lines_icebreakers['Arc91']['ice'] += piece_length / intVel
            lines_icebreakers['Arc92']['ice'] += piece_length / (intVel*0.9)
        elif intVel >= 9.5:
            lines_icebreakers['Arc91']['ice'] += piece_length / intVel
            lines_icebreakers['Arc92']['ice'] += piece_length / (intVel * 0.8)
        elif intVel > 0 and False:
            lines_icebreakers['Arc91']['ice'] += float('inf')
            lines_icebreakers['Arc92']['ice'] += float('inf')
        else:
            lines_icebreakers['Arc91']['clear'] += piece_length
            lines_icebreakers['Arc92']['clear'] += piece_length
    # print(lines)
    ...
    return lines, lines_icebreakers
...
if __name__ == "__main__":
    ...
    ice_data_sheets = pd.ExcelFile(xls).sheet_names[2:]  # Предполагаем, что первые два листа - lon и lat, остальные - данные о льде

    for ice_sheet in ice_data_sheets:
        first_week = pd.read_excel(xls, ice_sheet)
        ...
        port_name_to_coords = {row['point_name']: [row['latitude'], row['longitude']] for _, row in
                               points_df.iterrows()}
        ...
        for i, row in edges_df.iterrows():
            print(f"{ice_sheet} Ребро: {i}")
            map[int(row['id'])], map_icebreakers[int(row['id'])] = get_total_line_time_coefficient(port_id_to_coords[row['start_point_id']],
                                                                  port_id_to_coords[row['end_point_id']], row['length'])
            # if i == 5: break
        # print(map)
        date_file = (datetime.strptime(ice_sheet, "%d-%b-%Y")).strftime("%d.%m.")+"2022"
        json_file_path = "icemaps/test/ships/map_"+date_file+".json"
        dates.append(date_file)
        with open(json_file_path, 'w') as json_file:
            json.dump(map, json_file, indent=4, ensure_ascii=False)

        json_file_path = "icemaps/test/icebreakers/map_"+date_file+".json"
        with open(json_file_path, 'w') as json_file:
            json.dump(map_icebreakers, json_file, indent=4, ensure_ascii=False)


    with open("icemaps/test/sheets.json", 'w') as json_file:
        json.dump(dates, json_file, indent=4, ensure_ascii=False)