...
def solve_schedules_bot(sname, sclass, sspeed, sdep, sdest, sdate):
    all_times = []
    HOURS_FOR_WAIT = 36
    ...
    def get_all_data():
        ...
        schedule_df.append({'id': row[0]+1, 'ice_class': sclass,'ship_name': sname, 'velocity': sspeed, 'start_point_id': port_name_to_id[sdep], 'end_point_id': port_name_to_id[sdest], 'date_start': sdate, 'status': 1})
    ...
    all_routes = [route for route in all_routes if route['name'] == sname]
    ...
    def create_gantt_chart(routes):
        ...
        plt.savefig('../static/personal/gant_'+sname+'.png')
        print('saved')
    ...
    route_arrays = []
    for index, route in enumerate(all_routes[0]['route']):
        if index == 0:
            route_arrays.append(
                f"<b>{route['date']}</b>\n"
                f"<b>{route['port']}</b>\n"
            ...
            tmp = \
                f"<b>{route['date']}</b>\n" \
                f"<b>{route['port_name']}</b>\n"
            if route['waiting']:
                tmp += f"Ждем {math.floor(route['time'])} часов\n"
            else:
    ...
    def split_into_chunks(arr, chunk_size):
        """Разделить массив на подмассивы по chunk_size элементов."""
        return [arr[i:i + chunk_size] for i in range(0, len(arr), chunk_size)]

    def concatenate_chunks(chunks):
        """Конкатенировать подмассивы в строки."""
        return [' '.join(map(str, chunk)) for chunk in chunks]


    return '../static/personal/gant_'+sname+'.png', concatenate_chunks(split_into_chunks(route_arrays, 5))

if __name__ == '__main__':
    solve_schedules_bot("Тест", "Arc5", 15, "пролив Лонга", "Рейд Мурманска", datetime(2022, 3, 1))